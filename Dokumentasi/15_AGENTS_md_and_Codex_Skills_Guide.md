...
The system monitors Windows client devices running Accurate 5, sends structured telemetry to a VPS-based RSyslog server, parses the logs in Laravel, stores monitoring data in MySQL, reads Accurate/Firebird audit trail directly from Firebird, displays operational status in a dashboard, sends contextual Telegram alerts, and supports manual controlled remote actions.
...
MySQL monitoring database on VPS
...
Accurate Audit Reader -> Firebird AUDIT + USERS -> MySQL -> Accurate Audit Dashboard.
