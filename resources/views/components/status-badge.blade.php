...
@php
    ...
    $classes = match ($normalized) {
        'online', 'normal', 'running', 'connected', 'available', 'executed', 'resolved' => 'bg-green-100 text-green-700',
        'warning', 'pending', 'acknowledged' => 'bg-amber-100 text-amber-700',
...
<span {{ $attributes->merge(['class' => "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium uppercase {$classes}"]) }}>
    {{ str_replace('_', ' ', $normalized ?: 'unknown') }}
</span>
